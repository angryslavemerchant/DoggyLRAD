...
print(f"Detection: threshold {BARK_THRESHOLD}  |  ESP32 notify: {NOTIFY_ESP32}")
...
with sd.InputStream(
    ...
    while running:
        ...
        dog_scores = {i: float(scores[i]) for i in DOG_INDICES}
        best = max(dog_scores, key=dog_scores.get)
        if dog_scores[best] >= BARK_THRESHOLD:
            print(f"\n  !! BARK DETECTED -- {LABELS[best]} ({dog_scores[best]:.2f})")
            if NOTIFY_ESP32:
                notify_esp32()
